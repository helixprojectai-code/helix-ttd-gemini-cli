...
class NamingConvention:
    ...
    def validate(self, filename: str) -> Tuple[bool, List[str]]:
        ...
        if not parsed:
            violations.append(
                "Filename does not match Helix-TTD convention: ORIGIN-TYPE-SEQUENCE_DESCRIPTOR_YYYYMMDD[-REV{N}].ext"
            )
        ...
        if parsed.origin not in self.VALID_ORIGINS:
            violations.append(
                f"Unknown origin: {parsed.origin}. Valid: {', '.join(sorted(self.VALID_ORIGINS))}"
            )

        # Validate type
        if parsed.file_type not in self.VALID_TYPES:
            violations.append(
                f"Unknown type: {parsed.file_type}. Valid: {', '.join(sorted(self.VALID_TYPES))}"
            )
        ...
        if not re.match(r"^[a-z_]+$", parsed.descriptor):
            violations.append(
                f"Invalid descriptor: {parsed.descriptor}. Use lowercase and underscores only."
            )
        ...
        while True:
            ...
            if revision > 1:
                test_filename = test_filename.replace(
                    f".{filename.extension}", f"-REV{revision}.{filename.extension}"
                )
