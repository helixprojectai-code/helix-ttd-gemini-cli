...
def test_deepseek_receipt():
    ...
    # [TEST] Compute hash proof
    receipt.hash_proof = hashlib.sha256(json.dumps({
...
def test_deepseek_bridge():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        text = "[FACT] This is fact. [HYPOTHESIS] This is hypothesis. [ASSUMPTION] This is assumption."
...
def test_quorum_threshold_calculation():
    ...
    for num_attestations in [1, 2, 3]:
        ...
        assert receipt.quorum_reached == expected_quorum, f"Failed for {num_attestations} attestations"
