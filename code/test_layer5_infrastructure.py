...
def test_topological_distance():
    ...
    dist_kp = topology.get_topological_distance(
        ShlorpianCharacter.KORVO, ShlorpianCharacter.PUPA
    )
    assert 0 < dist_kp < 1  # Pupa is at center (0.5, 0.5)
    
    # [TEST] Korvo to Terry distance
    dist_kt = topology.get_topological_distance(
        ShlorpianCharacter.KORVO, ShlorpianCharacter.TERRY
    )
