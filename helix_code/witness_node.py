...
class OwlProtocol:
    ...
    def begin_session(self, custodian_id: str) -> WitnessEvent:
        ...
        state_hash = hashlib.sha256(
            f"session_start:{custodian_id}:{time.time()}".encode()
        ).hexdigest()

        event = self._create_event(
            WitnessType.OWL, state_hash, f"Session initiated for Custodian {custodian_id}"
        )
    ...
    def observe_operation(self, operation: str, drift_status: str) -> WitnessEvent:
        ...
        state_hash = hashlib.sha256(
            f"{operation}:{drift_status}:{time.time()}".encode()
        ).hexdigest()

        event = self._create_event(
            WitnessType.OWL, state_hash, f"Observed: {operation} | Drift: {drift_status}"
        )
        self.observations.append(event)
        self._persist_event(event)
    ...
    def _create_event(
        self, witness_type: WitnessType, state_hash: str, context: str
    ) -> WitnessEvent:
